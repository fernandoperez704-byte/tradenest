...
export default function PatternOverlay({
  ...
  const [overlayZone, setOverlayZone] = useState<OverlayZone | null>(null);
  ...
  useEffect(() => {
    ...
    if (!pattern || !chart || !candleSeries || !container) {
      setPosition(null);
      setOverlayPoints([]);
      setOverlayZone(null);
      return;
    ...
    function updatePosition() {
      ...
      if (!currentChart || !currentSeries || !currentContainer || !pattern) {
        setPosition(null);
        setOverlayPoints([]);
        setOverlayZone(null);
        return;
      ...
      const rawLeft = Math.min(startX, endX) - horizontalPadding;
      const rawRight = Math.max(startX, endX) + horizontalPadding;
      const rawTop = Math.min(highY, lowY) - verticalPadding;
      const rawBottom = Math.max(highY, lowY) + verticalPadding;

      const nextPosition: OverlayPosition = {
        left: rawLeft,
        top: rawTop,
        width: Math.max(40, rawRight - rawLeft),
        height: Math.max(40, rawBottom - rawTop),
      };
      ...
      const zoneHighY = currentSeries.priceToCoordinate(patternZone.high);
      const zoneLowY = currentSeries.priceToCoordinate(patternZone.low);

      if (zoneHighY == null || zoneLowY == null) {
        setOverlayZone(null);
        return;
      }

      setOverlayZone({
        top: Math.min(zoneHighY, zoneLowY) - rawTop,
        height: Math.max(2, Math.abs(zoneLowY - zoneHighY)),
      });
    }
  ...
  }, [pattern, chartInstanceRef, candleSeriesRef, chartContainerRef]);

  if (!pattern || !position || overlayPoints.length === 0) return null;

  const ascendingStart = overlayPoints.find((point) => point.label === "Start");
  const ascendingResistanceOne = overlayPoints.find(
    (point) => point.label === "Resistance 1"
  );
    ...
    <div className="pointer-events-none absolute inset-0 z-20 overflow-hidden">
      ...
      >
        ...
        >
          {overlayZone &&
            ...
            (pattern.type === "DOUBLE_TOP" || pattern.type === "DOUBLE_BOTTOM") && (
              <rect
                x={overlayPoints[1].x}
                y={overlayZone.top}
                width={Math.max(0, position.width - overlayPoints[1].x)}
          ...
          {pattern.type === "DOUBLE_TOP" && overlayPoints.length >= 5 && (
            <>
              <line
          ...
          {pattern.type === "DOUBLE_BOTTOM" && overlayPoints.length >= 5 && (
            <>
              <line
                x1={overlayPoints[1].x}
                y1={overlayPoints[1].y}
